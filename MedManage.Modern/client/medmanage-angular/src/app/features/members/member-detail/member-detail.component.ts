...
export class MemberDetailComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onSave(): void {
    ...
    const request: CreateMemberRequest = {
      ...
      dateOfBenefit: f.dateOfBenefit ? f.dateOfBenefit.toISOString().split('T')[0] : null,
      dateJoined: f.dateJoined ? f.dateJoined.toISOString().split('T')[0] : null,
      isMedAidExhausted: f.isMedAidExhausted ?? false,
      dateExhausted: f.dateExhausted ? f.dateExhausted.toISOString().split('T')[0] : null,
      isWaitingPeriod: f.isWaitingPeriod ?? false,
      isReinstated: f.isReinstated ?? false,
      dateReinstated: f.dateReinstated ? f.dateReinstated.toISOString().split('T')[0] : null,
      isDeceased: f.isDeceased ?? false,
      dateDeceased: f.dateDeceased ? f.dateDeceased.toISOString().split('T')[0] : null,

      countryId: f.countryId,
      passportNumber: f.passportNumber || null,
      passportExpiryDate: f.passportExpiryDate ? f.passportExpiryDate.toISOString().split('T')[0] : null,
      ...
      dateSuspended: f.dateSuspended ? f.dateSuspended.toISOString().split('T')[0] : null,
    ...
    const obs = this.isNew
      ...
      : this.memberService.update(this.memberId, { ...request, memberId: this.memberId });
