...
export class CaseWizardComponent implements OnInit, OnDestroy {
  ...
  addIcdCode(): void {
    const dialogRef = this.dialog.open(CodeLookupDialogComponent, {
      width: '700px',
      data: { codeType: 'icd', title: 'Search ICD Codes' }
    });

    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        // Determine type: first = primary, rest = secondary by default
        const type = this.icdCodes.length === 0 ? 'primary' : 'secondary';
        const typeChoice = prompt('Code type?\n1 = Primary\n2 = Secondary\n3 = Co-morbidity', type === 'primary' ? '1' : '2');
        const dateStr = prompt('Date of procedure (yyyy-MM-dd):', new Date().toISOString().split('T')[0]) || new Date().toISOString().split('T')[0];

        this.icdCodes.push({
          icdId: result.id,
          code: result.code,
          description: result.description,
          primaryCode: typeChoice === '1',
          secondaryCode: typeChoice === '2',
          coMorbidityCode: typeChoice === '3',
  ...
  onSubmit(): void {
    ...
    const request: CreateCaseRequest = {
      ...
      dateAdmitted: this.treatmentDates.length > 0 && this.treatmentDates[0].dateAdmitted ? this.treatmentDates[0].dateAdmitted : undefined,
      admissionTime: this.treatmentDates.length > 0 && this.treatmentDates[0].timeAdmitted ? this.treatmentDates[0].timeAdmitted : undefined,
      dateDischarged: this.treatmentDates.length > 0 && this.treatmentDates[0].dateDischarged ? this.treatmentDates[0].dateDischarged : undefined,
      dischargeTime: this.treatmentDates.length > 0 && this.treatmentDates[0].timeDischarged ? this.treatmentDates[0].timeDischarged : undefined
    };
    ...
    for (const icd of this.icdCodes) {
      try {
        await this.caseService.createIcd(caseId, {
          icdId: icd.icdId,
          primaryCode: icd.primaryCode,
          secondaryCode: icd.secondaryCode,
          dateOfProcedure: icd.dateOfProcedure
        }).toPromise();
      } catch { /* continue */ }
    }
  }
