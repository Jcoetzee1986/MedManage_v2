...
import { CaseLetterNoteDialogComponent } from '../case-letter-note-dialog/case-letter-note-dialog.component';
...
export class CaseDetailComponent implements OnInit, OnDestroy {
  ...
  private buildRequest(): CreateCaseRequest {
    ...
    return {
      ...
      caseStatusId: val.caseStatusId || this.caseData?.caseStatusId || this.caseData?.statusId || undefined,
      caseTypeId: val.caseTypeId || this.caseData?.authTypeId || undefined,
      caseCategoryId: val.caseCategoryId || this.caseData?.caseCategoryId || undefined,
      memberId: val.memberId || this.caseData?.memberId || undefined,
      referToId: val.referToId || this.caseData?.referToId || undefined,
      referFromId: val.referFromId || this.caseData?.referFromId || undefined,
  ...
  onPrintLetter(): void {
    ...
    // Open the letter note dialog first
    const dialogRef = this.dialog.open(CaseLetterNoteDialogComponent, {
      width: '600px',
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      ...
      // Save the letter note, then generate the PDF
      this.http.post(`/api/cases/${this.caseId}/letter-notes`, result)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            // Now generate the letter
            this.http.get(`/api/letter-templates/render/${this.caseId}`, { responseType: 'blob' })
              .pipe(takeUntil(this.destroy$))
              .subscribe({
                next: (blob) => {
                  ...
                  a.href = url;
                  a.download = `CaseLetter_${this.caseData?.authNumber || this.caseId}_${new Date().toISOString().split('T')[0]}.pdf`;
                  a.click();
                  window.URL.revokeObjectURL(url);
                  this.snackBar.open('Case letter generated', 'Close', { duration: 3000 });
                },
                error: () => this.snackBar.open('Failed to generate case letter.', 'Close', { duration: 5000 })
              });
          },
          error: () => this.snackBar.open('Failed to save letter note.', 'Close', { duration: 3000 })
        });
    });
  }
