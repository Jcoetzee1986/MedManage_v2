...
export class CaseLinkedCasesTabComponent implements OnInit {
  ...
  items: CaseLinkDto[] = [];
  displayedColumns = ['linkedCaseNumber', 'linkedCaseMember', 'actions'];
  showAddForm = false;
  ...
  onAdd(): void {
    ...
    const request: CreateCaseLinkRequest = { childCaseId };
  ...
  onNavigate(item: CaseLinkDto): void {
    const targetId = item.childCaseId === this.caseId ? item.parentCaseId : item.childCaseId;
    this.router.navigate(['/cases', targetId]);
  }

  onDelete(item: CaseLinkDto): void {
    if (confirm('Remove this link?')) {
      this.caseService.deleteLink(this.caseId, item.id).subscribe({
        next: () => {
          this.loadItems();
